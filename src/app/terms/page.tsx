...
const ITEMS = [
  ...
  { titleKey: "terms.ordersTitle", textKey: "terms.ordersText" },
  { titleKey: "terms.paymentTitle", textKey: "terms.paymentText" },
  { titleKey: "terms.returnsTitle", textKey: "terms.returnsText" },
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-orange-50/30">
      <div className="max-w-5xl mx-auto px-3 sm:px-4 py-10 sm:py-14">
        <div className="relative rounded-3xl overflow-hidden shadow-2xl mb-12 h-56 sm:h-72 ring-4 ring-white/50">
          <Image src="https://images.unsplash.com/photo-1586281380349-632531db7b4a?w=900&h=500&fit=crop" alt={t("terms.title")} fill className="object-cover" priority />
          <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/40 to-transparent flex items-end p-8">
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold text-white drop-shadow-lg">{t("terms.title")}</h1>
              <p className="text-slate-200 text-sm sm:text-base mt-1">{t("terms.subtitle")}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-lg p-6 sm:p-10 mb-8">
          ...
          <p className="text-slate-500 text-sm mt-8">{t("terms.updateNote")}</p>
        </div>
