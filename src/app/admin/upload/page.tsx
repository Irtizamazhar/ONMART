...
export default function AdminUploadPage() {
  ...
  useEffect(() => {
    fetch("/api/store/categories")
      .then((r) => r.json())
      .then((data) => setCustomCategories(Array.isArray(data) ? data : []))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (numPrice > 9999999999.99) {
      setMessage({ type: "err", text: "Price is too large (max 9,999,999,999.99)." });
      return;
    }
    setSubmitting(true);
    ...
    try {
      ...
      const r = await fetch("/api/store/products", {
        ...
        body: JSON.stringify({
          ...
          sectionSlug: sectionSlug.trim() || undefined,
          inStock,
          ...(sectionSlug && sectionSlug.trim() && discountPercent.trim() !== "" && !Number.isNaN(Number(discountPercent)) && Number(discountPercent) >= 0 && Number(discountPercent) <= 100 ? { discountPercent: Math.round(Number(discountPercent)) } : {}),
        }),
      });
      if (!r.ok) {
      ...
      setMessage({ type: "ok", text: "Product uploaded with " + collectedImages.length + " image(s)." });
      setTitle("");
      setPrice("");
      setImageUrls([""]);
      setImageFiles([null]);
      ...
      <div className="max-w-lg bg-white rounded-lg border border-gray-200 shadow-sm p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Product will appear in this section on the home page.</p>
          </div>
          {sectionSlug && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Discount % (optional – store par isi % se badge dikhega)
              </label>
              <input
                type="number"
                min={0}
                ...
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-800"
              />
              <p className="text-xs text-gray-500 mt-0.5">Store par isi % se discount dikhega.</p>
            </div>
          )}
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="inStock" className="text-sm font-medium text-gray-700">In Stock (uncheck for Out of Stock)</label>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Product images (1–5): upload from laptop or paste URL. First = main.
            </label>
            {imageUrls.map((url, i) => (
